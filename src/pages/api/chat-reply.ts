...
export default async function handler(
  ...
  const { content, username, persona }: { content: string; username: string; persona?: Persona } = req.body;
  ...
  try {
    ...
    const aiReply = data.choices?.[0]?.message?.content?.trim() || 'Erro ao gerar resposta.';
    ...
    if (dbError) {
      ...
      return res.status(500).json({ error: 'Failed to save AI reply to database.' });
    }

    return res.status(200).json({ message: 'Reply sent successfully.', aiReply });
